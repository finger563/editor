...
class CelllarAutomaton(QtGui.QGraphicsItem):
...
class MainWindow(QtGui.QWidget):
    def __init__(self, parent=None):
        ...
        validator = QtGui.QIntValidator(0,1)
        ruleLayout = QtGui.QGridLayout()
        ruleLayout.setAlignment(QtCore.Qt.AlignTop)
        self.ruleEdits = []
        for i in range(7,-1,-1):
            ruleEdit = QtGui.QLineEdit()
            ruleEdit.setValidator(validator)
            ruleEdit.setText("0")
            ruleEdit.setFixedWidth(30)
            ruleEdit.textEdited.connect(self.update_rule)
            ruleLayout.addWidget(QtGui.QLabel("{0:03b}".format(i)), 0, 7-i)
            ruleLayout.addWidget(ruleEdit, 1,7-i)
            self.ruleEdits.append(ruleEdit)

        validator2 = QtGui.QIntValidator(0,255)
        self.rule10Edit = QtGui.QLineEdit()
        self.rule10Edit.setValidator(validator2)
        self.rule10Edit.textEdited.connect(self.update_rule10)
        rule10Layout = QtGui.QHBoxLayout()
        rule10Layout.addWidget(QtGui.QLabel("Rule"))
        rule10Layout.addWidget(self.rule10Edit)

        self.resetButton = QtGui.QPushButton("&Reset")
        self.resetButton.clicked.connect(self.reset)
        self.randomInitButton = QtGui.QPushButton("&Random init")
        ...
        propertyLayout.setAlignment(QtCore.Qt.AlignTop)
